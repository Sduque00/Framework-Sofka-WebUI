...
public class Helper {
    ...
    public static ContactUs generateContactUs(String language, String country, String emailDomain){
        Faker faker =  Faker.instance(
                new Locale(language, country),
                new Random()
        );
        ...
        contactUs.setHeading("Webmaster");
        contactUs.setEmail(faker.name().username().concat(emailDomain).replace(SPACE_STRING, EMPTY_STRING));
        contactUs.setOrderReference("Reference");
        contactUs.setMessage(faker.lorem().sentence());
